...
fsExtra.renameSync(join(targetFolder, newFilesName, config.templateFolder + '.md'), join(targetFolder, newFilesName, newFilesName+'.md'))
...
function writeFilePromise(path, data) {
  return new Promise((res,rej)=>fs.writeFile(path, data, 'utf8', e =>{
    if(e) rej(e)
    res();
  }))
}